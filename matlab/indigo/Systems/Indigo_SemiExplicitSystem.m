...
%
classdef Indigo_SemiExplicitSystem < Indigo_BaseSystem
  %
  methods
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    ...
    %
    function this = Indigo_SemiExplicitSystem( t_name, t_neqn, t_ninv )
      this@Indigo_BaseSystem(t_name, t_neqn, t_ninv);
    end
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    ...
    function [JF_x, JF_x_dot] = JF( this, x, x_dot, t )
      JF_x     = -this.Jf_x(x, t); % - this.Jf_v(x, v, t) * this.Jv_x(x, t);
    ...
    %
    function out = Jf_x( this, x, x_dot, t )
      TA  = this.TA_x(x, t);
      out = zeros(length(x));
      for i = 1:size(TA, 3)
        out(:,i) = TA_x(:,:,i) * x_dot;
      end
      out = this.A(x, t) \ (this.Jb_x(x, t) - out);
    end
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    ...
    %> \return The Jacobian \f$ \mathbf{Jf}_{\mathbf{x}} \f$..
    %
    %function out = Jf_v( this, x, x_dot, t )
    %  TA  = this.TA_v(x, t);
    %  out = zeros(length(x));
    %  for i = 1:size(TA, 3)
    %    out(:,i) = TA_v(:,:,i) * x_dot;
    %  end
    %  out = this.A(x, t) \ (this.Jb_v(x, t) - out);
    %end
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
