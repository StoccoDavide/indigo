...
ODE = LinearPendulumODE_Implicit( m, l, g );
...
for i = 2:length(solver_name)
  eval(strcat([ 'plot( T', solver_name{i}, ', Z', solver_name{i}, '(1,:), ''--'', ''LineWidth'', linewidth );' ]));
end
...
ylabel('Error (--)');
for i = 2:length(solver_name)
  eval(strcat([ 'plot( T', solver_name{i}, ', log(abs(ZExact(1,:)-Z', solver_name{i}, '(1,:))), ''--'', ''LineWidth'', linewidth );' ]));
end
