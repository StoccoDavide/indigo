% +--------------------------------------------------------------------------+
% | 'Indigo' module version 1.0 - BSD 3-Clause License - Copyright (c) 2023  |
% | Current version authors:                                                 |
% |   Davide Stocco and Enrico Bertolazzi.                                   |
% +--------------------------------------------------------------------------+

% Matlab generated code for implicit system: Index4
% This file has been automatically generated by Indigo.
% DISCLAIMER: If you need to edit it, do it wisely!

classdef Index4 < Indigo.DAE.Implicit
  %
  % No class description provided.
  %
  properties (SetAccess = protected, Hidden = true)
    % User data
    % No Properties
  end
  %
  methods
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function this = Index4( varargin )
      % Class constructor.

      % Superclass constructor
      num_eqns = 6;
      num_veil = 0;
      num_invs = 4;
      this = this@Indigo.DAE.Implicit('Index4', num_eqns, num_veil, num_invs);
    end % Index4
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_F = F( ~, in_1, in_2, ~, t )
      % Evaluate the function F.

      % Extract properties
      % No properties

      % Extract inputs
      x__2 = in_1(2);
      x__3 = in_1(3);
      x__4 = in_1(4);
      x__5 = in_1(5);
      x__6 = in_1(6);
      x__1_dot = in_2(1);
      x__2_dot = in_2(2);
      x__3_dot = in_2(3);
      x__4_dot = in_2(4);
      x__5_dot = in_2(5);
      x__6_dot = in_2(6);

      % Evaluate function
      t1 = cos(t);
      out_1 = x__1_dot - t1 + x__2 - x__3;
      out_2 = x__2_dot - t1 + x__3 - x__4;
      t2 = sin(t);
      out_3 = x__5_dot - t1 - t2 + x__6;
      out_4 = x__4_dot - t1 + x__5 - x__6;
      out_5 = x__3_dot - t1 + x__4 - x__5;
      out_6 = 2 * x__5_dot - x__6_dot - x__4_dot;

      % Store outputs
      out_F = zeros(6, 1);
      out_F(1) = out_1;
      out_F(2) = out_2;
      out_F(3) = out_3;
      out_F(4) = out_4;
      out_F(5) = out_5;
      out_F(6) = out_6;
    end % F
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_JF_x = JF_x( ~, ~, ~, ~, ~ )
      % Evaluate the Jacobian of F with respect to x.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      out_1_2 = 1;
      out_1_3 = -1;
      out_2_3 = 1;
      out_2_4 = -1;
      out_5_4 = 1;
      out_4_5 = 1;
      out_5_5 = -1;
      out_3_6 = 1;
      out_4_6 = -1;

      % Store outputs
      out_JF_x = zeros(6, 6);
      out_JF_x(1, 2) = out_1_2;
      out_JF_x(1, 3) = out_1_3;
      out_JF_x(2, 3) = out_2_3;
      out_JF_x(2, 4) = out_2_4;
      out_JF_x(5, 4) = out_5_4;
      out_JF_x(4, 5) = out_4_5;
      out_JF_x(5, 5) = out_5_5;
      out_JF_x(3, 6) = out_3_6;
      out_JF_x(4, 6) = out_4_6;
    end % JF_x
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_JF_x_dot = JF_x_dot( ~, ~, ~, ~, ~ )
      % Evaluate the Jacobian of F with respect to x_dot.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      out_1_1 = 1;
      out_2_2 = 1;
      out_5_3 = 1;
      out_4_4 = 1;
      out_6_4 = -1;
      out_3_5 = 1;
      out_6_5 = 2;
      out_6_6 = -1;

      % Store outputs
      out_JF_x_dot = zeros(6, 6);
      out_JF_x_dot(1, 1) = out_1_1;
      out_JF_x_dot(2, 2) = out_2_2;
      out_JF_x_dot(5, 3) = out_5_3;
      out_JF_x_dot(4, 4) = out_4_4;
      out_JF_x_dot(6, 4) = out_6_4;
      out_JF_x_dot(3, 5) = out_3_5;
      out_JF_x_dot(6, 5) = out_6_5;
      out_JF_x_dot(6, 6) = out_6_6;
    end % JF_x_dot
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_JF_v = JF_v( ~, ~, ~, ~, ~ )
      % Evaluate the Jacobian of F with respect to v.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      % No body

      % Store outputs
      out_JF_v = zeros(6, 0);
    end % JF_v
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_v = v( ~, ~, ~ )
      % Evaluate the the veils v.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      % No body

      % Store outputs
      out_v = zeros(0, 1);
    end % v
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_Jv_x = Jv_x( ~, ~, ~, ~ )
      % Evaluate the Jacobian of v with respect to x.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      % No body

      % Store outputs
      out_Jv_x = zeros(0, 6);
    end % Jv_x
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_h = h( ~, in_1, ~, t )
      % Calculate the residual of the invariants h.

      % Extract properties
      % No properties

      % Extract inputs
      x__1 = in_1(1);
      x__2 = in_1(2);
      x__3 = in_1(3);
      x__4 = in_1(4);
      x__5 = in_1(5);
      x__6 = in_1(6);

      % Evaluate function
      t1 = sin(t);
      out_1 = -x__1 - x__2 - x__3 - x__4 - x__5 + 5 * t1;
      out_2 = -x__2 + t1;
      out_3 = -x__3 + x__4;
      out_4 = 2 * x__5 - x__6 - x__4;

      % Store outputs
      out_h = zeros(4, 1);
      out_h(1) = out_1;
      out_h(2) = out_2;
      out_h(3) = out_3;
      out_h(4) = out_4;
    end % h
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_Jh_x = Jh_x( ~, ~, ~, ~ )
      % Calculate the Jacobian of h with respect to x.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      out_1_1 = -1;
      out_1_2 = -1;
      out_2_2 = -1;
      out_1_3 = -1;
      out_3_3 = -1;
      out_1_4 = -1;
      out_3_4 = 1;
      out_4_4 = -1;
      out_1_5 = -1;
      out_4_5 = 2;
      out_4_6 = -1;

      % Store outputs
      out_Jh_x = zeros(4, 6);
      out_Jh_x(1, 1) = out_1_1;
      out_Jh_x(1, 2) = out_1_2;
      out_Jh_x(2, 2) = out_2_2;
      out_Jh_x(1, 3) = out_1_3;
      out_Jh_x(3, 3) = out_3_3;
      out_Jh_x(1, 4) = out_1_4;
      out_Jh_x(3, 4) = out_3_4;
      out_Jh_x(4, 4) = out_4_4;
      out_Jh_x(1, 5) = out_1_5;
      out_Jh_x(4, 5) = out_4_5;
      out_Jh_x(4, 6) = out_4_6;
    end % Jh_x
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out_Jh_v = Jh_v( ~, ~, ~, ~ )
      % Calculate the Jacobian of h with respect to v.

      % Extract properties
      % No properties

      % Extract inputs

      % Evaluate function
      % No body

      % Store outputs
      out_Jh_v = zeros(4, 0);
    end % Jh_v
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
    function out = in_domain( ~, ~, ~ )
      out = true;
    end % in_domain
    %
    % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
    %
  end
end % Index4

% That's All Folks!
