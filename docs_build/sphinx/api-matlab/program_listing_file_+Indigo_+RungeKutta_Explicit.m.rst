...
.. _program_listing_file_+Indigo_+RungeKutta_Explicit.m:

Program Listing for File Explicit.m
===================================

|exhale_lsh| :ref:`Return to documentation for file <file_+Indigo_+RungeKutta_Explicit.m>` (``+Indigo/+RungeKutta/Explicit.m``)
...
.. code-block:: MATLAB
   ...
   %> \f]
   %
   classdef Explicit < Indigo.RungeKutta.Method
     %
     methods
       %
       % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
       ...
       %
       function this = Explicit( name, order, tbl )
       ...
       function out = step_residual( this, i, x_i, K, v_i, t_i )
         ...
         out = this.m_sys.F(x_i, K, v_i, t_i);
       end
       %
       % - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
       %
       ...
       function [out, ierr] = solve_step( this, x_k, K_0, t_k, d_t )
         ...
         explicit = this.is_explicit() && ...
                    (this.m_sys.is_explicit() || this.m_sys.is_semiexplicit());
         nc = length(this.m_c);
         K  = repmat(K_0, 1, nc);
         ...
         for i = 1:nc
           ...
           if (explicit)
             ...
             fun = @(K_i) this.step_residual(i, x_i, K_i, v_i, t_i);
             jac = @(K_i) this.step_jacobian(i, x_i, K_i, v_i, t_i);
     ...
     methods (Static)
       ...
       function out = check_tableau( tbl )
         ...
         CMD = 'Indigo.RungeKutta.Methods.Explicit.check_tableau( tbl ): ';
